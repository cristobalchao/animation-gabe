...
window.onload = function() {
  ...
  $(document).on('click', function(){
    ...
    stopVideo = (!!stopVideo)?false:true;
  });
...
var init = function() {
  ...
  renderer.setSize( $('#top').width(), $('#top').height() );
...
var render = function () {
  ...
  camera.position.x += ( mouseX - camera.position.x ) * .05;
        camera.position.y += ( - mouseY - camera.position.y ) * .05;1
  ...
  for (var i = 0; i < num_logo_arcs; i++) {
    ...
    for ( var a = 0; a < num_particles_per_arc; a++ ) {
      ...
      if (!!!stopVideo){
        if ( videoPos >= videoMax ){
          reversePlay = true;
        }else if ( videoPos <= videoMax * -1 ){
          reversePlay = false;
        }

        videoPos = (!!!reversePlay)?videoPos + videoSpeed:videoPos - videoSpeed;
      }
