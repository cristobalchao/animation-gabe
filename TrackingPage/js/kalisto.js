...
(function( window, $, undefined ){
	...
	$.fn.animate = function(){
		...
		$this.addClass($kalisto.stopClass);
	...
	function init(){
		...
		$($kalisto.element).each(function(){
			...
			$(this).css({'position':'absolute','translate':[0,$kalisto.arrayPositions[i++]]}).delay(500)
			.queue(function() {
				$(this).setDuration($kalisto.duration);
			});
		});

		var heightContainer = $kalisto.arrayPositions[$kalisto.arrayPositions.length-1] + parseFloat($($kalisto.element).last().css('height'))

		$kalisto.container.css('height',heightContainer+'px');

		//Triggering the elements ON CLICK
		$($kalisto.element).on('click',function(){
			if (!!!$kalisto.container.hasClass('active') && !!!$(this).hasClass($kalisto.stopClass)){
				$(this).animate();
			}else if ($(this).hasClass($kalisto.stopClass) && !!!$kalisto.container.hasClass('active')){
				$(this).removeClass($kalisto.stopClass);
			}
		});
	}
	...
	$.fn.kalisto = function(args) {
		var $this = this,
			...
			_stopClass = (!!args.stopClass) && args.stopClass || "";
